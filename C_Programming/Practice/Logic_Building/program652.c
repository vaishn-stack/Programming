...
void CreateDILB()
{
    ...
    for(i = 1; i <= MAXINODE; i++)
    {
        ...
        memset(newn->FileName,0,sizeof(newn->FileName));
        newn -> FileSize = 0;
               ...
               )
{
    ...
    uareaobj.UFDT[i]->ptrinode->Buffer = (char *)calloc(MAXFILESIZE,sizeof(char));
              ...
              )
{
    ...
    {
        uareaobj.UFDT[fd]->WriteOffset = MAXFILESIZE;
    }
    ...
    if(uareaobj.UFDT[fd]->WriteOffset > uareaobj.UFDT[fd]->ptrinode->ActualFileSize)
    {
        uareaobj.UFDT[fd]->ptrinode->ActualFileSize =
        uareaobj.UFDT[fd]->WriteOffset;
    }

    return size;   // how many bytes data we can written
}
        ...
        else if(iCount == 2)
        {
            ...
            else if(strcmp(Command[0],"write") == 0)
            {
                ...
                if(iRet == ERR_INVALID_PARAMETER)
                {
                    ...
                    printf("Please refer man page of valid parameters");
                }
